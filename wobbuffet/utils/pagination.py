...
class Pagination:
    ...
    async def from_bot(cls, ctx):
        ...
        def sort_key(cmd):
            ...
            category = category_cfg["index"] if category_cfg else category
        ...
        entries = sorted(ctx.bot.commands, key=sort_key)
        nested_pages = []
        per_page = 15

        for cog, commands in itertools.groupby(entries, key=group_key):
