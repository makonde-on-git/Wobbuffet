...
from wobbuffet.utils import snowflake

get_id = snowflake.create()

LOGGERS = ('wobbuffet_logs', 'discord_logs')

module_logger = logging.getLogger('wobbuffet.logger')
...
def init_logger(bot, debug_flag=False):
    ...
    # setup bot logger
    wobbuffet_log = logging.getLogger("wobbuffet")
    ...
    wobbuffet_fh = create_fh('wobbuffet.log')
    wobbuffet_fh.setLevel(logging.INFO)
    wobbuffet_fh.setFormatter(log_format)
    wobbuffet_log.addHandler(wobbuffet_fh)
    ...
    wobbuffet_console = logging.StreamHandler(console_std)
    wobbuffet_console.setLevel(logging.INFO if debug_flag else logging.ERROR)
    wobbuffet_console.setFormatter(log_format)
    wobbuffet_log.addHandler(wobbuffet_console)
    ...
    # create db handler
    wobbuffet_db = DBLogHandler(bot, 'wobbuffet_logs')
    bot.log_wobbuffet_db = wobbuffet_db
    wobbuffet_log.addHandler(wobbuffet_db)
    discord_db = DBLogHandler(bot, 'wobbuffet_logs')
    discord_log.addHandler(discord_db)

    bot.add_cog(ActivityLogging(bot))

    return wobbuffet_log
