...
from wobbuffet import command, group, checks
...
class CogManager:
    ...
    async def load_extension(self, ctx, name, extension):
        ...
        if ext not in self.available_exts and ext_loc == 'wobbuffet.exts':
    ...
    async def _list(self, ctx):
        ...
        all_extensions = self.available_exts
        not_emoji = "\N{BLACK SMALL SQUARE}"
        is_emoji = "\N{WHITE SMALL SQUARE}"
        status_list = []
        loaded = []
        for ext in all_extensions:
            is_loaded = f"wobbuffet.exts.{ext}" in ctx.bot.extensions
            if is_loaded:
                status = is_emoji
                loaded.append(ext)
            else:
        ...
        await ctx.info(
            f'Available Extensions - {len(loaded)}/{len(all_extensions)}',
            '\n'.join(status_list))
    ...
    async def unload(self, ctx, extension):
        """Unload an extension."""
        ext_name = f"wobbuffet.exts.{extension}"
    ...
    async def load(self, ctx, *extensions):
        ...
        for ext in extensions:
            ...
            await self.load_extension(ctx, name, f"wobbuffet.exts.{ext}")

    @load.command(name="core")
    async def _core(self, ctx):
        """Reload Core Commands."""
        await self.load_extension(
            ctx, 'Core Commands', f'wobbuffet.core.commands')

    @load.command(name="cm")
    async def _cm(self, ctx):
        """Reload Cog Manager."""
        await self.load_extension(
            ctx, 'Cog Manager', f'wobbuffet.core.cog_manager')
